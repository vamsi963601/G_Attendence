...
class _HomePageState extends State<HomePage> {
  ...
  Future<String> _checkLogin(String uid, String pass) async {
    ...
    http.Response response = await http
        .post(Uri.encodeFull(url), body: {"user_id": uid, "password": pass});
  ...
  Future<String> getCoursesData() async {
    final String url =
        'http://3.6.138.40/GAtt/api/modules/test.php?/=getCourses';
    final String url1 =
        'http://3.6.138.40/GAtt/api/modules/test.php?/=getTodayAtt';
    var sp = await SharedPreferences.getInstance();
    String uid = sp.getString("user_id");
    http.Response response =
        await http.post(Uri.encodeFull(url), body: {"user_id": uid});
    http.Response response1 =
        await http.post(Uri.encodeFull(url1), body: {"user_id": uid});
    Map<String, dynamic> user = jsonDecode(response.body);
